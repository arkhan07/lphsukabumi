<x-layouts.admin.app>
    <x-slot name="title">Edit Konfigurasi Biaya</x-slot>

    <!-- Page Header -->
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h2 class="mb-1" style="font-size: 1.75rem; font-weight: 600;">Edit Konfigurasi Biaya</h2>
            <p class="text-secondary-light mb-0">Perbarui konfigurasi biaya sertifikasi halal</p>
        </div>
        <div>
    ...
    <form action="{{ route('admin.finance.fees.update', $fee->id) }}" method="POST">
        @csrf
        @method('PUT')
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-12 col-md-6">
                            ...
                            <select class="form-select @error('category') is-invalid @enderror" name="category" required>
                                ...
                                <option value="certification" {{ old('category', $fee->category) == 'certification' ? 'selected' : '' }}>Sertifikasi</option>
                                <option value="audit" {{ old('category', $fee->category) == 'audit' ? 'selected' : '' }}>Audit</option>
                                <option value="renewal" {{ old('category', $fee->category) == 'renewal' ? 'selected' : '' }}>Perpanjangan</option>
                                <option value="inspection" {{ old('category', $fee->category) == 'inspection' ? 'selected' : '' }}>Inspeksi</option>
                                <option value="administrative" {{ old('category', $fee->category) == 'administrative' ? 'selected' : '' }}>Administrasi</option>
                                <option value="other" {{ old('category', $fee->category) == 'other' ? 'selected' : '' }}>Lainnya</option>
                            </select>
                            @error('category')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-12">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror"
                                      name="description"
                                      rows="3"
                                      placeholder="Jelaskan detail biaya ini...">{{ old('description', $fee->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="row g-3">
                        <div class="col-12 col-md-6">
                            ...
                            <select class="form-select @error('fee_type') is-invalid @enderror"
                                ...
                                <option value="fixed" {{ old('fee_type', $fee->fee_type) == 'fixed' ? 'selected' : '' }}>Tetap (Fixed)</option>
                                <option value="percentage" {{ old('fee_type', $fee->fee_type) == 'percentage' ? 'selected' : '' }}>Persentase (%)</option>
                                <option value="variable" {{ old('fee_type', $fee->fee_type) == 'variable' ? 'selected' : '' }}>Variabel</option>
                            </select>
                            @error('fee_type')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="row g-3">
                        <div class="col-12 col-md-6">
                            ...
                            <input type="date"
                                   ...
                                   name="effective_date"
                                   value="{{ old('effective_date', $fee->effective_date ? $fee->effective_date->format('Y-m-d') : '') }}"
                                   required>
                        ...
                        <div class="col-12 col-md-6">
                            ...
                            <input type="date"
                                   ...
                                   name="expiry_date"
                                   value="{{ old('expiry_date', $fee->expiry_date ? $fee->expiry_date->format('Y-m-d') : '') }}">
                            @error('expiry_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="row g-3">
                        <div class="col-12">
                            <div class="form-check form-switch">
                                <input class="form-check-input"
                                       ...
                                       name="is_taxable"
                                       value="1"
                                       {{ old('is_taxable', $fee->is_taxable) ? 'checked' : '' }}>
                        ...
                        <div class="col-12">
                            ...
                            <textarea class="form-control @error('notes') is-invalid @enderror"
                                      name="notes"
                                      rows="3"
                                      placeholder="Catatan untuk admin...">{{ old('notes', $fee->notes) }}</textarea>
                            @error('notes')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                ...
                <div class="card-custom mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('is_active') is-invalid @enderror" name="is_active">
                            <option value="1" {{ old('is_active', $fee->is_active) == '1' ? 'selected' : '' }}>Aktif</option>
                            <option value="0" {{ old('is_active', $fee->is_active) == '0' ? 'selected' : '' }}>Tidak Aktif</option>
                        </select>
                        @error('is_active')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div class="card-custom mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <p class="mb-0" style="font-weight: 500;">
                            {{ $fee->created_at ? $fee->created_at->format('d M Y H:i') : '-' }}
                        </p>
                    </div>
                    @if($fee->updated_at && $fee->updated_at != $fee->created_at)
                    <div class="mb-3">
